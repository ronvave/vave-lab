...
| # | Test | Steps |
|---|---|---|
| 2 | Role enforcement | `ronvave2011@gmail.com` added to `Admin Users` as `Researcher`. Sign in as that account in a separate browser/Incognito profile — confirm it can view but not edit/approve/rollback/publish. Then test removal/deactivation reverting to "Not authorised." |
| 10–12 | Second-authenticator flow, dual-authenticator identity decision, conflicting-decision escalation | Re-role `ronvave2011@gmail.com` to `Authenticator` for these. Run one identity case through both authenticator logins agreeing, then a case where the two logins record disagreeing decisions, and confirm it escalates rather than auto-resolving. |
