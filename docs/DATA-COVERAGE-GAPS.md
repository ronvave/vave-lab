...
| **Confederacy · Province rollup (Panel B2 subview)** | `scholars{}` + admin province data | New scholar rolls into their province / confederacy tile once Ron fills in province data on the admin side. **Confederacies are sorted descending by total Masters + PhD theses on every refresh** (largest on top; alphabetical tiebreak). A 4th **Untagged** row below the 3 confederacies always shows Masters / PhD theses that couldn't be joined to a paternal or maternal province, using the same M / PhD / Total layout. The **`BY CONFEDERACY`** label is appended with `Total N | Masters N | PhD N` for the 3 confederacies only — the Untagged row is not included in that suffix. Every count refreshes automatically at each 3-hour refresh or force sync (`js/itaukei-database.js` → `renderWorldPanelConfederacyView`). |
| **Country + University lists (Panel B2 dropdowns)** | `worldPoints[]` | New country / university appears in the alphabetized dropdown automatically. **Each country row shows the number of distinct universities in parentheses next to the country name** (Variant A styling, approved 2026-07-23) — e.g. `Australia (23)` — rendered in the rust Masters KPI colour so it reads as a stat, not part of the linked country name. Count is `c.unis.length` where `c.unis` is the pipeline's array of distinct `{country, university}` leaves under the Zotero Thesis root; it refreshes on every render, so the 3-hour refresh and force sync keep it in sync automatically. |
...
| 2026-07-28 | Alias-chain repair + transitive resolver + trailing-period auto-alias | Ron: "why does Esther Batiri-Williams appear twice (despite only appearing once (no variants) in Admin dashboard so there's no option to merge). Also, there's a total of 9 variants that I've merged several times but it keeps showing back up. Fix it." | Two root causes: (a) `deriveScholarRows` in `js/itaukei-database.js` iterates BOTH Zotero sub-collections AND scholar profiles, so a sub-collection named `Batiri-Williams, Esther` with no alias entry emitted a duplicate card next to the canonical profile `Williams, Esther Batiri` (admin doesn't show the duplicate because admin only iterates Zotero creators — sub-collection names never appear as creator strings). (b) `nameAliases` contained 11 multi-hop chains and 2 self-loops (`Navia, Miliana` → itself; `King, Temalesi Vere` → `King, Temalesi V.` inside a cycle), and the single-hop resolvers in both `admin.js` (`resolveAlias`) and `itaukei-database.js` (`canonicalizeCreator`, `resolveSubName`) only followed one edge — so a chain `A → B → C` folded A into B but never all the way into C, keeping the variant panel populated with already-merged pairs. Fix: (1) `scripts/repair_name_aliases.py` runs a one-shot transitive closure with a best-canonical scoring heuristic (fully-typed given name > initial-only; is-scholar-profile > not; longer name > shorter; alpha tie-break) that mirrors admin.js's `detectVariantGroups`, drops self-loops, and adds sub-collection→profile aliases via token-set match (`{batiri, williams, esther}` matches both `Batiri-Williams, Esther` and `Williams, Esther Batiri`). Also auto-aliases 5 trailing-period Zotero data-entry variants that Ron reported "keep coming back" (`Carlton, Lauren H` → `Carlton, Lauren H.`, plus Moreland / Turley / Parkes / Hill, Philip C). (2) All three resolvers now walk the chain to a fixed point with a 32-hop cap and cycle guard. (3) `applyVariantMergeInMemory` in `admin.js` now collapses every existing alias chain at merge time, so a subsequent `B → D` merge automatically re-points any prior `A → B` alias at D without waiting for another repair pass. Alias count: 537 → 541 (19 changes: 11 chain collapses, 2 self-loops removed, 6 new sub-collection + period-pair entries). |
