...
7. From the function dropdown at the top, run **backupSolomonReviewDataV2**, then **setupSolomonSubmissionQueues**, then **authorizeScholarSubmissionStorage**. Approve the requested Google permissions. These create a private backup, private queues/upload storage, and storage for submissions. The 81 current scholar share tokens are already initialized in Master. For future scholars with missing links, run **initializeSolomonShareTokens**; it preserves existing links. Existing scholar data remains unchanged. Do not make the uploads folder publicly shared.
