/**
 * Passcode gate for the Solomon Islands Scholar Database.
 *
 * This is a fully separate, sister clone of js/db-gate.js built for the
 * Solomon Islands Scholar Database admin/dashboard system. It uses its own brand-new
 * passcode and its own VERIFIER_HASH_HEX \u2014 it never reuses the iTaukei/Solomon Islands
 * passcode, hash, or ENC_FILES map. It is not read by, and does not read,
 * the iTaukei/Solomon Islands localStorage session key.
 *
...
(function () {
  ...
  var STORAGE_KEY = 'solomonlab.db.session.v1';
  ...
  //   passcode = "Waelava7!"  -- SET/ROTATE THIS before real data goes live
  ...
  var VERIFIER_HASH_HEX =
    '5fdf38ce4005a9a9308d6ed63fb96c1d7662912c6c68af572fe0b66bee4480b8';

  // Every file the Solomon Islands database fetches. Maps the plaintext .json URL
  // that solomon-database-adapter.js / admin-solomon-islands-master.js request onto
  ...
  var ENC_FILES = {
    ...
    // These are consumed by solomon-islands-research-database-master.html /
    // js/solomon-database-master.js and refreshed by the Solomon Islands variant of
    // the every-2h refresh-master-file.yml workflow. The plaintext .json
    // paths are gitignored; only the .enc blobs are committed.
  ...
  function buildLockScreenHtml() {
    return ''
      ...
      +     '<p class="db-gate__lede">This Solomon Islands Scholar Database is being previewed with a small group of colleagues while feedback is gathered. If Prof. Ron Vave has shared a passcode with you, enter it below.</p>'
