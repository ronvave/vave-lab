/* ============================================================================
 * admin-tongan-master.js  —  Vave Lab Tongan Scholar Database Admin (V2)
 *
 * Sister clone of js/admin-master.js. Every CRUD control/tab/filter/
 * validation is preserved exactly, except: Province/Confederacy relabeled
 * to District/Island Division; Village/Town (Kolo) and Specific Island are
 ...
 * Fiji-specific vocabulary (province list, island division list) is replaced by
 * the Tonga District / Island-Division lookups; Scholar IDs are stable
 * `TON-S####` (never iTaukei's `ITK-S####`). CRUD joins by stable Scholar ID
 * string, never by row number \u2014 same as the iTaukei admin.
 *
 * Purpose
 * -------
 * Manage the admin-owned supplementary data that overlays the Master Google
 * Sheet for the Tongan Scholar dashboard:
 *
 *   - data/tongan-scholar-enrichment.json.enc      (Scholar-ID keyed)
 *   - data/tongan-scholar-insights-master.json.enc (Scholar-ID keyed)
 *   - img/scholars/<TON-Sxxxx>.jpg          (400×400 in-browser resize)
 *
 ...
 * js/tongan-db-gate.js with its own brand-new passcode/verifier hash and its
 * own ENC_FILES map — fully separate from the iTaukei gate). Every push
...
(function () {
  ...
  // `refresh-tongan-master-file.yml` GitHub Action; without a dispatch, the public
  ...
  // Province → Confederacy pattern exactly) and, once write-back is
  // enabled, to drive dependent dropdowns for Paternal District → Paternal
  // Island Division and Maternal District → Maternal Island Division.
  // Tongatapu / Vava'u / Ha'apai / 'Eua / Ongo Niua are the five canonical
  // Island Divisions (2021 Census, Tonga Statistics Department). Sourced
  // from the approved crosswalk (tongan_scholar_database_crosswalk.md §5).
  var DISTRICT_TO_DIVISION = {
    "Kolofo'ou": 'Tongatapu', "Kolomotu'a": 'Tongatapu', 'Vaini': 'Tongatapu',
    'Tatakamotonga': 'Tongatapu', 'Lapaha': 'Tongatapu', 'Nukunuku': 'Tongatapu',
    'Kolovai': 'Tongatapu',
    'Neiafu': "Vava'u", 'Pangaimotu': "Vava'u", 'Hahake': "Vava'u",
    "Leimatu'a": "Vava'u", 'Hihifo': "Vava'u", 'Motu': "Vava'u",
    'Pangai (Lifuka)': "Ha'apai", 'Foa': "Ha'apai", 'Lulunga': "Ha'apai",
  ...
  var ENRICHMENT_URL = 'data/tongan-scholar-enrichment.json';
  var INSIGHTS_URL   = 'data/tongan-scholar-insights-master.json';
  var ENRICHMENT_ENC = 'data/tongan-scholar-enrichment.json.enc';
  var INSIGHTS_ENC   = 'data/tongan-scholar-insights-master.json.enc';
  ...
  async function loadOldZoteroCountsBySid () {
    ...
    try {
      scholarProfiles = await window.dbGate.fetchJson('data/tongan-scholar-profiles.json');
    } catch (e) {
    ...
    // tongan-scholar-profiles.json is name-keyed (V1 shape). Match to Master rows by
  ...
  function renderScholars () {
    ...
    if (state.filterIslandDivision) {
      rows = rows.filter(function (s) {
        return String(s['Island Division'] || '').trim() === state.filterIslandDivision;
      });
    }
  ...
  function sortValue (s, key) {
    ...
    switch (key) {
      ...
      case 'islandDivision':  return String(s['Island Division'] || '');
  ...
  function scholarRowHtml (s) {
    ...
    var conf = (s['Island Division'] || '').trim();
  ...
  function renderGradDegreesRows (sid, rows, container) {
    ...
    var fields = [
      ...
      { name: 'Country',                     label: 'Country' },
      { name: 'International from Tonga?',   label: 'International from Tonga?' },
    ...
    try {
      ...
      results.forEach(function (r, j) {
        ...
        if (r.status === 'ok') {
          ...
          // 2-hour cron. The `refresh-tongan-master-file.yml` dispatch below
  ...
  async function performNonMasterPush (sid, opts) {
    ...
    try {
      ...
      // present in tongan-scholar-enrichment.json.enc are preserved verbatim so a
      ...
      //    sha race, so concurrent refresh-tongan-master-file runs (salt-only
      ...
      if (needsPublicRefresh) {
        log('Master-backed public field changed — dispatching refresh-tongan-master-file.yml…', 'warn');
  ...
  //           handles the salt-only rewrite that refresh-tongan-master-file.yml
  ...
  //       the refresh-tongan-master-file workflow ran twice (12:55 and 13:00) and
  //       rewrote tongan-scholar-enrichment.json.enc with fresh salts. The old
  ...
  async function pushEncryptedJsonMerged (path, plaintextUrl, applyEdit, commitMsg) {
    ...
    // than blindly re-PUTting stale bytes. With refresh-tongan-master-file.yml
  ...
  //
  // Then step 2: dispatch refresh-tongan-master-file.yml (the Tongan
  // sister workflow to iTaukei's refresh-master-file.yml, once deployed)
  // so any Master Sheet edits in the same session are picked up in the
  // next snapshot.
  //
  // Files scanned:
  //   - tongan-research-database-master.html  (public V2 dashboard, once built)
  //   - admin-tongan-master.html               (this admin, so admin
  ...
  // file that contains Tongan diacritics such as the 'okina ('\u02bb) and
  // macrons, or the middle-dot separator). The GitHub Contents API
  // returns/expects base64 of the raw byte sequence.
  ...
  function computeRepairPriority (canonicalTotal, oldTotal, reason) {
    ...
    // repair value: e.g. a scholar TON-S0379 with old 68 / current 0.
  ...
  function gapRows () {
    ...
    state.scholars.forEach(function (s) {
      ...
      rows.push({
        ...
        islandDivision:    s['Paternal Island Division'] || s['Island Division'] || '',
  ...
  function exportGapsCsv () {
    ...
    var header = [
      ...
      'Alive/Deceased', 'Discipline', 'Paternal Island Division', 'Roster Tier'
    ];
    ...
    a.href = url;
    a.download = 'tongan-master-authorship-linkage-gaps-' + new Date().toISOString().slice(0, 10) + '.csv';
  ...
  function wireControls () {
    ...
    if (migWriteBtn) migWriteBtn.addEventListener('click', async function () {
      ...
      if (!confirm('Publish ' + migState.lastResult.counts.v2TotalAfter + ' Scholar-ID-keyed insights to data/tongan-scholar-insights-master.json.enc?')) return;
      migWriteBtn.disabled = true;
      try {
        await window.adminInsightsMigration.publishV2(migState.lastResult.v2Doc);
        toast('Published data/tongan-scholar-insights-master.json.enc.', 'ok');
      } catch (e) {
    ...
    $('#filter-island-division').addEventListener('change', function (e) {
      state.filterIslandDivision = e.target.value; renderScholars();
    });
    ...
    // Refresh from Sheet (top bar). Dispatches refresh-tongan-master-file.yml only
