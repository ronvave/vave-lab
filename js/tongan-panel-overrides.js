/*
 * Tongan Master-file panel overrides
 * ===================================
 *
 * Sister clone of js/master-file-panel-overrides.js (iTaukei). The
 * production dashboard code in js/tongan-database-master.js was forked
 * from js/itaukei-database-master.js and now runs against Tongan
 * Master-file data synthesized into Zotero shape by
 * js/tongan-database-adapter.js. Almost every panel works verbatim. This
 * file layers on the *Master-file-specific* requirements the Zotero shape
 * cannot express directly:
 *
 *   1. The two district summary tables (B1 + C2) show the 23 Tonga
 *      districts grouped by Island Division WITH dedicated TOTAL columns
 *      per division (Tongatapu total, Vava'u total, Ha'apai total, 'Eua
 ...
 *      non-Tongan records appear verbatim below the summary tables.
...
(function () {
  ...
  // -------------------------------------------------------------------
  // 1. District × Island Division TOTAL columns for B1 and C2
  // -------------------------------------------------------------------
  //
  ...
  // inside each Island Division" requirement, we render an additional
  // summary table into the two panel hosts.
  //
  // NOTE: the underlying adapter (js/tongan-database-adapter.js) keeps the
  // internal constant names CONFEDERACIES / PROVINCE_TO_CONFED / PROVINCES
  // for logic compatibility with the cloned dashboard JS — but their
  // *contents* are Tonga's 5 Island Divisions and 23 Districts, never a
  // Fijian confederacy. This file reads those same constants and displays
  // them exclusively under Tonga-appropriate labels.

  function tallyByProvinceAndConfed(scope /* 'all' | 'tongan' */) {
  ...
  // Stable display order + CSS-safe slug for Tonga's 5 Island Divisions.
  var DIVISION_ORDER = ['Tongatapu', "Vava'u", "Ha'apai", "'Eua", 'Ongo Niua'];
  var DIVISION_SLUG = {
    'Tongatapu': 'tongatapu',
    "Vava'u": 'vavau',
    "Ha'apai": 'haapai',
  ...
  function renderConfedTotalTable(hostEl, tally, scope) {
    ...
    var Mfc = window.MasterFileAdapter.constants;
    var scopeLabel = scope === 'tongan'
      ? 'Tongan-associated publications'
      : 'All Tonga publications';

    var html = '';
    html += '<table class="mf-confed-table" role="table" aria-label="' + scopeLabel + ' by district and Island Division">';
    html += '<caption class="mf-confed-table__cap">' + scopeLabel +
            ' \u2014 districts grouped by Island Division, with division totals</caption>';
    html += '<thead><tr><th scope="col" class="mf-th-confed">Island Division</th>' +
  ...
  function injectStylesOnce() {
    ...
    st.textContent =
      // Island Division totals table styling — matches production teal/coral/gold tokens.
      ...
      // Five Tonga Island Division badge colors (distinct from the iTaukei
      // 3-confederacy palette; chosen from the same teal/coral/gold family
      // plus two additional neutral tones for the extra two divisions).
      '.mf-badge--tongatapu { background:#e8f2f3; color:#005f6b; }' +
      ...
      // Empty-state note used for panels with no Tongan data yet.
  ...
  function injectConfedTotals() {
    ...
    // Panel B (Tonga districts): the production chart is inside .db-panel--b .db-panel__body.
    ...
    if (bPanel) {
      ...
      title.textContent = 'B \u00b7 Tonga publications by district & Island Division (Master-file authoritative)';
      ...
      if (tally && anyNonZero(tally.byProvince)) {
        ...
        subhost.innerHTML = '<p class="mf-empty-state"><strong>Data not yet available.</strong> '
          + 'District-tagged Tonga publications have not been populated in the Master file yet. '
    ...
    // Panel C2 (research in and across Tonga's 23 districts).
    ...
    if (c2Panel) {
      ...
      title2.textContent = 'C2 \u00b7 Tongan-associated publications by district & Island Division';
      var subhost2 = document.createElement('div');
      var tally2 = tallyByProvinceAndConfed('tongan');
      if (tally2 && anyNonZero(tally2.byProvince)) {
        renderConfedTotalTable(subhost2, tally2, 'tongan');
      } else {
        subhost2.innerHTML = '<p class="mf-empty-state"><strong>Data not yet available.</strong> '
          + 'No Tongan-associated, district-tagged publications have been populated yet. '
          + 'This panel will populate automatically once records are added.</p>';
      }
      host2.innerHTML = '';
