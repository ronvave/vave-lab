...
(function () {
  ...
  if (!root) return;
  let model = null, view = 'gender', warning = ''; 
  ...
  function render() {
    ...
    table.querySelector('caption').textContent = 'Graduate degree short disciplines — unique Tongan scholars with completed degrees' + (gender ? ' by gender' : '');
    ...
    table.hidden = false;
    table.tFoot.hidden = !model.rows.length;
    if (!model.rows.length) {
      const tr = document.createElement('tr'), td = document.createElement('td');
      td.colSpan = 4;
      td.textContent = model.allCompleted
        ? `Short Discipline classifications are not yet recorded for the ${number(model.allCompleted)} eligible scholars with completed degrees. Discipline counts will appear after the Master file is updated.`
        : 'No eligible completed graduate degrees are recorded in the current snapshot.';
  ...
  function hydrate(master) {
    ...
    if (!next || !Array.isArray(next.rows) || !next.overall) {
      warning = model ? 'The latest discipline summary is unavailable. Showing the last loaded data.' : 'The discipline summary is unavailable. Please reload after the next Master data refresh.';
      status.textContent = warning;
      return;
    }
    warning = ''; model = next; render();
  }
  setTimeout(() => {
  ...
  window.addEventListener('vavelab:master-hydrated', e => hydrate(e.detail && e.detail.master));
  if (window.__masterHydrated) hydrate(window.__vavelabDbState && window.__vavelabDbState.master);
})();
