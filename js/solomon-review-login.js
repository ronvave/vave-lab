/* Solomon review-only access. Credentials stay in memory and are sent only in POST bodies. */
(function(){
'use strict';
const CLIENT_ID=window.SolomonSubmissionConfig?.googleClientId||'';
...
async function signedIn(result){
 ...
 try{
  ...
  if(caps.country!=='Solomon Islands'||caps.status!=='ok'||!['owner','admin'].includes(caps.role))throw new Error('Google Admin access is not enabled for this account yet.');
  ...
  if(!document.getElementById('queue-script')){
   const script=document.createElement('script');script.id='queue-script';script.src='js/solomon-submissions-admin.js?v=google-roles-1';
...
if(!CLIENT_ID||!ENDPOINT){message('Google review sign-in awaits Solomon Islands backend configuration. The Owner panel remains available.');return;}
let attempts=0;
