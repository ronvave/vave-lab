/* Solomon-only review UI. Master validation and private review journal live in Apps Script. */
(function () {
'use strict';
...
function locationText(l){return l.national?'Solomon Islands — general / national study':[l.province,l.island,l.village].filter(Boolean).join(' · ')||'Solomon Islands';}
function chips(parent,title,values){const group=el('div',null,parent);el('strong',title,group);const wrap=el('div',null,group);if(!values.length)el('span','—',wrap).className='meta';values.forEach(v=>el('span',v,wrap).className='solomon-location-chip');}
function countries(s){return String(s||'').split(';').map(x=>x.trim()).filter(Boolean);}
async function refreshPublic(){if(window.SolomonSubmissionAdmin?.refreshMessage)return window.SolomonSubmissionAdmin.refreshMessage;try{return window.SolomonSubmissionAdmin&&await window.SolomonSubmissionAdmin.refresh({silent:true})?'Public snapshot refresh queued; publication is not yet verified.':'Master saved; public snapshot refresh was not dispatched. Use Refresh from Sheet.';}catch(e){return 'Master saved; public refresh failed: '+e.message;}}
document.querySelectorAll('[data-solomon-queue]').forEach(host=>{
 const kind=host.dataset.solomonQueue,scholar=kind==='scholar';
 const tab=document.querySelector('[data-tab="'+(scholar?'scholar-submissions':'geography-submissions')+'"]');
 const badge=el('span',null,tab);badge.className='solomon-submission-badge';badge.hidden=true;badge.setAttribute('aria-live','polite');
 ...
 const bulk=el('span',null,toolbar);bulk.className='solomon-bulk-controls';bulk.hidden=true;
 button('Check All',bulk,()=>cards.forEach(c=>{if(c.select)c.select.checked=true;}));button('Clear All',bulk,()=>cards.forEach(c=>{if(c.select)c.select.checked=false;}));button('Approve all checked',bulk,bulkResolve,'solomon-approve');
 const count=el('span','',toolbar);count.className='solomon-queue-count';
 const status=el('p','Open this tab to load submissions.',host);status.className='solomon-queue-status';status.setAttribute('role','status');
 const list=el('div',null,host);
 function message(text,error){status.textContent=text;status.classList.toggle('solomon-error',!!error);}
 ...
 async function load(after){if(busy)return;busy=true;refresh.disabled=true;filter.disabled=true;message('Loading…');const saved=savedState();try{
   if(!window.adminWriteback||!window.adminWriteback.isConfigured())throw new Error('Configure the Solomon endpoint and shared secret in Data source & GitHub first.');
 ...
 function render(row,saved){
  ...
  card.className='solomon-review solomon-review--'+String(row.Status||'').toLowerCase();
  const top=el('div',null,card);top.className='solomon-review-heading';
  ...
  el('span',row.Status,top).className='solomon-status-pill';
  const identity=el('p',null,card);identity.className='solomon-submitter';identity.append('Submitted by: '+(row['Submitter Name']||'Unknown')+(row.Relationship?' ('+row.Relationship+')':'')+' · ');link(row['Submitter Email'],'mailto:'+row['Submitter Email'],identity);if(row['Profile URL']){identity.append(' · ');link('Submitted scholar profile',row['Profile URL'],identity);}
  if(scholar)renderScholar(c,pending);else{
    const grid=el('div',null,card);grid.className='solomon-geography-grid';
    chips(grid,'Current Solomon Islands locations',(row.currentSolomonLocations||[]).map(locationText));
    if(!Array.isArray(row.currentSolomonLocations))el('p',row.currentGeographyError||'Current locations unavailable until the Solomon backend is updated. No locations have been inferred.',grid).className='solomon-error';
    chips(grid,'Proposed Solomon Islands locations',parse(row['Proposed Solomon Islands Locations JSON'],[]).map(locationText));chips(grid,'Proposed Pacific countries',countries(row['Proposed Pacific Countries']));chips(grid,'Proposed other countries',countries(row['Proposed Other Countries']));
  }
  if(!pending){el('p',[row.Resolution,row['Review Notes'],row['Reviewed By'],row['Reviewed At']].filter(Boolean).join(' · '),card);return;}
  const label=el('label','Optional review note',card);label.className='solomon-note-label';c.note=el('textarea',null,label);c.note.value=saved?.note||row.reviewPlan?.note||'';c.note.setAttribute('aria-label','Review note for '+row['Submission ID']);
  const actions=el('div',null,card);actions.className='solomon-review-actions';
  if(scholar){
    const ban=button('Ban submitter',actions,()=>banSubmitter(c),'solomon-ban');ban.disabled=!caps.banSubmitter;if(ban.disabled)ban.title=caps.role==='admin'?'Only the Owner can ban submitters.':'Requires verified Solomon review backend v2.';
    const approve=button(row.reviewPlan?'Resume checked review':'Approve all checked',actions,()=>approveScholar(c),'solomon-approve');approve.disabled=!caps.combinedReview;
    if(!caps.combinedReview)el('p','Combined approval requires Solomon Apps Script review backend v2. Queue reads and secure downloads remain available.',card).className='solomon-error';
  }else button('Approve',actions,()=>resolveOne(c,'approve'),'solomon-approve');
  button(scholar?'Reject all':'Reject',actions,()=>resolveOne(c,'reject'),'solomon-reject');
 ...
 function renderScholar(c,pending){
  ...
  if(pending){const controls=el('div',null,card);controls.className='solomon-selection-controls';button('Check all',controls,()=>c.picks.forEach(p=>{if(!p.box.disabled)p.box.checked=true;}));button('Clear all',controls,()=>c.picks.forEach(p=>{if(!p.box.disabled)p.box.checked=false;}));}
  ...
  }else el('p','No text differences from the live Master file. Review any attachments below.',disclosure).className='solomon-positive';
  ...
  const files=parse(row['Attachments JSON'],[]),attachments=el('section',null,card);attachments.className='solomon-attachments';el('strong','Attachments ('+files.length+')',attachments);
  files.forEach(f=>{
    const item=el('div',null,attachments);item.className='solomon-attachment';const outcome=plan?.items.find(i=>i.kind==='file'&&i.key===f.fileId);
 ...
 async function run(c,fn){if(busy)return;busy=true;const controls=[...host.querySelectorAll('button,select,input,textarea')],disabled=controls.map(x=>x.disabled);controls.forEach(x=>x.disabled=true);try{const text=await fn();busy=false;await load(text);}catch(e){message(e.message,true);}finally{busy=false;controls.forEach((x,i)=>x.disabled=disabled[i]);if(c?.row.reviewPlan){c.picks.forEach(p=>p.box.disabled=true);const resume=c.card.querySelector('.solomon-review-actions .solomon-approve');if(resume)resume.textContent='Resume checked review';}refreshBadge();}}
     ...
     for(const item of plan.items.filter(x=>x.kind==='file'&&x.state==='pending')){
       ...
       try{
         if(p.file.field==='headshot'){
           const result=await window.SolomonSubmissionAdmin.approvePhoto(c.row['Scholar ID'],await p.data(),id+':'+p.key);
 ...
 async function banSubmitter(c){const reason=prompt('Reason for blocking this submitter in Solomon:');if(!reason?.trim()||!confirm('Block future Solomon submissions from '+c.row['Submitter Email']+'? Reason: '+reason))return;await run(c,async()=>{checked(await window.adminWriteback.banScholarSubmitter(c.row['Submission ID'],reason));return 'Submitter blocked for Solomon. The submission remains available for review.';});}
