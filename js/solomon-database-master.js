/*
 * Solomon Islands Scholar Database
 *
...
(function () {
  ...
  const PIN_OFFSETS = {
    ...
    'Santa Catalina': [0, 0],
    'South Ulawa': [0, 0],
    'Star Harbour North': [0, 0],
    'Star Harbour South': [0, 0],
    'Ugi and Pio': [0, 0],
    'Wainoni East': [0, 0],
    'Wainoni West': [0, 0],
    'Weather Coast': [0, 0],
    ...
    'Fouenda': [0, 0],
    'Gulalofou': [0, 0],
    'Keaimela/Radefasu': [0, 0],
    'Kwarekwareo': [0, 0],
    'Langalanga': [0, 0],
    'Luaniua': [0, 0],
    'Malu\'u': [0, 0],
    'Mandalua/Folotana': [0, 0],
    'Mareho': [0, 0],
    'Matakwalao': [0, 0],
    'Nafinua': [0, 0],
    'Pelau': [0, 0],
    ...
    'Siesie': [0, 0],
    'Sikaiana': [0, 0],
    'Sububenu/Burianiasi': [0, 0],
    'Sulufou/Kwarande': [0, 0],
    'Tai': [0, 0],
    'Takwa': [0, 0],
    'Waneagu Silana Sina': [0, 0],
    'Waneagu/Taelanasina': [0, 0],
    'West Baegu/Fataleka': [0, 0],
    'East Gaongau': [0, 0],
    'East Tenggano': [0, 0],
    'Kanava': [0, 0],
    'Lughu': [0, 0],
    ...
    'Luva Station': [0, 0],
    'Manuopo': [0, 0],
    'Nanggu/Lord Howe': [0, 0],
    'Nea/Noole': [0, 0],
    'Nenumpo': [0, 0],
    'Neo': [0, 0],
    'Nevenema': [0, 0],
    'Nipua/Nopoli': [0, 0],
    'North East Santa Cruz': [0, 0],
    'Polynesian Outer Islands': [0, 0],
    'Tikopia': [0, 0],
    'Utupua': [0, 0],
    'Vanikoro': [0, 0],
    'Central Ranongga': [0, 0],
    'Gizo': [0, 0],
    'Inner Shortlands': [0, 0],
    'Irringgilla': [0, 0],
    'Kolombaghea': [0, 0],
    'Kusaghe': [0, 0],
    'Mbilua': [0, 0],
    ...
    'Nono': [0, 0],
    'Noro': [0, 0],
    'North Kolombangara': [0, 0],
    'North Ranongga': [0, 0],
    'North Rendova': [0, 0],
    'North Vangunu': [0, 0],
    'Nusa Roviana': [0, 0],
    'Outer Shortlands': [0, 0],
    'Roviana Lagoon': [0, 0],
    'Simbo': [0, 0],
    'South Kolombangara': [0, 0],
    'South Ranongga': [0, 0],
    'South Rendova': [0, 0],
    'Vonavona': [0, 0],
    'Vonunu': [0, 0],
  ...
  // is blank. Used for provinceGroup chips, Panel B2 grouping, and every place
  ...
  function findItaukeiRootCollection(cols) {
    ...
    return cols.find(c => !c.parent && /^Solomon Islands authors\b/i.test(String(c.name || ''))) || null;
  }
  ...
  async function loadAll() {
    ...
    // Explicit hide-list — names the admin dashboard has removed from the iTaukei/Solomon Islands list.
    ...
    const itaukeiParents = snap.collections.filter(c =>
      c.name === 'By or with Solomon Islands authors' || c.name.startsWith('Solomon Islands authors')
    );
    ...
    const byWith = snap.collections.find(c => c.name === 'By or with Solomon Islands authors');
    if (byWith) itaukeiParentKeys.add(byWith.key);

    // Recursively add every descendant under the iTaukei/Solomon Islands Thesis tree.
    ...
    const thesisRoot = snap.collections.find(c => c.key === '9XHGQJE6')
                    || snap.collections.find(c => c.name === 'B2-Solomon Islands Thesis by Country/Universities')
                    || snap.collections.find(c => c.name === 'Solomon Islands Thesis by Country/Universities');
    if (thesisRoot) {
    ...
    // etc.) and would otherwise pollute the iTaukei/Solomon Islands set. Filtering here keeps
  ...
  // Dynamic paragraph + four insight blocks for the iTaukei/Solomon Islands narrative card.
  function renderTopNarrative(x) {
    ...
    if (bodyEl) {
      ...
      parts.push('Together, this scholarship spans diverse fields and connects with communities across Solomon Islands and internationally.');
  ...
  function initMap() {
    try {
      ...
      state.map = map;
      renderChoropleth();
      // Fullscreen toggle for the Solomon Islands choropleth map. On expand we
      // re-fit to the district layer bounds so the map re-centers on
      // Solomon Islands at the wider aspect ratio; on collapse we restore.
      // Reset-view button for the Solomon Islands choropleth map. Fits back to the
      // stored default bounds and closes any open popup. Same defaults are
      // reused for inline and fullscreen — the bounds already frame Solomon Islands
  ...
  // Province publication breakdown (Panel A choropleth + provinceGroup legend numbers).
  ...
  // Module-scope profile lookup used by the provinceGroup/province rollup so it
  ...
  // provinceGroup view only needs a Boolean + provinces, no photo/village work.
  ...
  // ProvinceGroup · Province rollup for Panel B2 tabulated summary.
  ...
  // groups by provinceGroup. Scholars with no matched profile / no province
  ...
  function renderWorldPanelProvinceGroupView(host) {
    if (!host) return;
    const { confRows, provRows, totals } = buildB2ProvinceGroupRollup();
    const parts = [];
    ...
    // Single white card holds all three provinceGroup rows + the
    ...
    // province, and therefore to a provinceGroup). Same visual pattern
    // as the provinceGroup rows so Ron sees at a glance how many theses
  ...
  function applyWorldListView() {
    ...
    const confHost = listView.querySelector('[data-world-provinceGroup-list]');
  ...
  function renderWorldPanel() {
    ...
    // country/university/provinceGroup/province selection. Ron's July 2026
    ...
    // Apply the active list view (country vs provinceGroup). This shows/hides
    // the country list + search vs the provinceGroup rollup, and re-renders the
  ...
  function refreshFilteredBadges() {
    ...
    // Scholar-panel filter (name, keyword, sector, discipline, provinceGroup,
  ...
  // Frame dropdown selections to the marker copies actually rendered on the
  // map. Using the current viewport centre to normalize longitude can select
  // an adjacent wrapped copy of a country/region where no scholar markers
  // exist, leaving a large empty map after filtering.
  ...
  function zoomToWorldUniversity(uniName) {
    ...
    if (rendered.length) {
      ...
      state.worldMap.setView(rendered[0], p ? (p.locationPrecision === 'country' ? 4 : 8) : 8, { animate: true });
      return;
    }
    const grad = state.graduateStudies || { worldPoints: [] };
    const p = (grad.worldPoints || []).find(x => x.university === uniName);
    ...
    state.worldMap.setView([p.lat, lng], p.locationPrecision === 'country' ? 4 : 8, { animate: true });
  }
  ...
  function initWorldMap() {
    ...
    if (worldWrap && worldResetBtn) {
      worldResetBtn.addEventListener('click', (e) => {
        ...
        const m = state.worldMap; if (!m) return;
        m.closePopup();
        // Clear every fullscreen filter (provinceGroup, province, sector,
        ...
        // the provinceGroup dropdown — refresh its label/selection UI when
  ...
  // ProvinceGroup for a scholar name (uses paternalProvince, then maternalProvince).
  // Profile.provinceGroup may be the literal string 'Unclassified' — treat that
  // as "not set" so the province → provinceGroup fallback still fires.
  function scholarConfProv(name) {
    ...
    let provinceGroup = (p && p.provinceGroup || '').trim();
    if (!provinceGroup || provinceGroup.toLowerCase() === 'unclassified') {
      provinceGroup = PROVINCE_TO_CONFEDERACY[province] || '';
    }
    return { profile: p, province, provinceGroup };
  }

  // Return true if a scholar name passes ALL the active fullscreen filters
  // (provinceGroup, province, sector, work-country, work-institution).
  function scholarPassesWorldFilters(name) {
    ...
    const { profile, province, provinceGroup } = scholarConfProv(name);

    if (conf && provinceGroup !== conf) return false;
  ...
  // note in the toolbar explains the gap). Also applies the provinceGroup /
  ...
  function buildWorkplacePoints() {
    ...
    profiles.forEach((p) => {
      ...
      // Apply the cross-filters (provinceGroup / province / sector). These
  ...
  // The cross-filter predicate for Workplace mode. Runs the provinceGroup,
  ...
  function _scholarPassesCrossFilters(nameFirstLast, profile) {
    ...
    const sector = state.worldSectorFilter;
    if (conf) {
      const c = (profile.provinceGroup || '').trim();
      if (c !== conf) {
        // Try inferring from province if provinceGroup was blank.
  ...
  // attachments): provinceGroup-tinted top bar with the institution name,
  ...
  const _WORK_CONF_BAR = {
    Central: 'linear-gradient(90deg, #FF5A6E 0%, #c93e50 100%)',
    Choiseul: 'linear-gradient(90deg, #4ECDE6 0%, #0891b2 100%)',
    Guadalcanal: 'linear-gradient(90deg, #FFD84A 0%, #f7b500 100%)',
    Isabel: 'linear-gradient(90deg, #6FCF97 0%, #2f9e5c 100%)',
    'Makira-Ulawa': 'linear-gradient(90deg, #B48CE6 0%, #7c4dbd 100%)',
    Malaita: 'linear-gradient(90deg, #F2994A 0%, #c9701f 100%)',
  ...
  function _workplaceVillageLine(p) {
    ...
    // paternal. We keep the provinceGroup tag on the maternal note so a
    // mixed-heritage scholar still surfaces the maternal provinceGroup.
    let matNote = '';
    if (mat && mat !== pat) {
  ...
  function buildWorkplacePopupHtml(point) {
    ...
    const cards = scholars.map(p => {
      ...
      // never lose the provinceGroup context.
    ...
    return (
      `<div class="db-work-popup">` +
        ...
        `</div>` +
        `<div class="db-work-popup__count">${total} · Solomon Islander scholar${total === 1 ? '' : 's'} working here</div>` +
  ...
  // Rebuild the provinceGroup dropdown label + selection markers to reflect
  ...
  // Compute scholar counts per provinceGroup and per province across the
  ...
  function computeProvinceGroupScholarCounts() {
    ...
    (grad.worldPoints || []).forEach(p => {
      ...
      names.forEach(n => {
        const { province, provinceGroup } = scholarConfProv(n);
        const cf = provinceGroup || 'Unclassified';
  ...
  function wireWorldMapFilters() {
    ...
    function renderConfList() {
      ...
      renderProvList(state.worldConfFilter || 'Central');
    }
    ...
    function renderProvList(confKey) {
      ...
      if (!confKey || confKey === 'Unclassified') {
        ...
        provList.innerHTML = confKey === 'Unclassified'
          ...
          : '<div style="padding:10px 12px;color:#71717a;font-size:0.9rem;">Hover a provinceGroup to see its provinces.</div>';
        return;
      }
      provTitle.textContent = `Provinces in ${confKey}`;
      const provinces = CONFEDERACY_PROVINCES[confKey] || [];
      const { provCounts } = computeProvinceGroupScholarCounts();
    ...
    // Study mode. Cross-filters (provinceGroup / sector) stay applied
    ...
    function zoomToRegion(countries) {
      ...
      if (!latlngs.length) {
        const grad = state.graduateStudies || { worldPoints: [] };
        const pts = (grad.worldPoints || []).filter(p => set.has(p.country) && Number.isFinite(p.lat) && Number.isFinite(p.lng));
    ...
    state.provinceLayer = L.geoJSON(geo, {
      ...
      style: (feature) => {
        ...
        return {
          ...
          fillOpacity: 0.92,
          color: CONF_COLORS[feature.properties.provinceGroup] || '#333',
  ...
  function makeProvincePopup(p, b) {
    ...
    const scholarLine = b.scholars > 0
      ? `<p class="db-popup-meta" style="margin-top:8px;padding-top:6px;border-top:1px dashed #cbd5e1;"><span style="font-weight:700;color:${CONF_COLORS[p.provinceGroup]};font-variant-numeric:tabular-nums;">${b.scholars}</span> Solomon Islander scholar${b.scholars === 1 ? '' : 's'} on this map</p>`
      : '';
  ...
  // ============ PANEL A — map + legend + provinceGroup tallies + sync ============
  function renderPanelA() {
    ...
    // ProvinceGroup tallies. For each provinceGroup we track three counts:
    //   total    — all Solomon Islands-focused publications (district-tagged), regardless of authorship
    ...
    const byConf = {
      Central: { total:0, itaukei:0, coauth:0 },
      Choiseul: { total:0, itaukei:0, coauth:0 },
      Guadalcanal: { total:0, itaukei:0, coauth:0 },
      Isabel: { total:0, itaukei:0, coauth:0 },
      'Makira-Ulawa': { total:0, itaukei:0, coauth:0 },
      Malaita: { total:0, itaukei:0, coauth:0 },
    ...
    // Dynamic narrative sentence beneath the provinceGroup rows. Ranks the three
    ...
    if (narrativeEl) {
      ...
      const [a, b, c] = ranked;
      let s = `Solomon Islands-focused publications are most concentrated in ${a.name} (${a.total.toLocaleString()}), followed by ${b.name} (${b.total.toLocaleString()}) and ${c.name} (${c.total.toLocaleString()}). `;
  ...
  function renderPanelB() {
    ...
    // No provinceGroup dot on the label; an "i" tooltip explains the category.
    ...
    if (nonProv.total > 0) {
      // Column 1 — label + info icon (no provinceGroup dot).
  ...
  // ============ PANEL D — provinceGroup small multiples ============
  function renderPanelD() {
    ...
    confs.forEach(cf => {
      const provInCf = provs.filter(p => p.provinceGroup === cf)
  ...
  function getPanelDData() {
    ...
    // The roster is defined by distinct canonical SOL-S#### Scholar IDs.
    const scholarsById = new Map();
    (Array.isArray(master.scholars) ? master.scholars : []).forEach(s => {
      const id = String(s['Scholar ID'] || '').trim();
      if (/^SOL-S\d{4}$/.test(id) && !scholarsById.has(id)) scholarsById.set(id, s);
    });
    ...
    // Solomon Islands schema field names: the Solomon Islands sheet has no 'Province Paternal'
    // or 'Paternal ProvinceGroup' column at all (see crosswalk §4) —
    // 'Ward Paternal' IS the direct Fiji-Province equivalent (Solomon Islands'
    ...
    // holds Solomon Islands' Ward Paternal value and is displayed with the
    ...
    // Degree milestones are derived solely from the current Master Scholars and Graduate Degrees.
  ...
  state.scholarPage = 1;
  state.scholarConfFilter = '';  // '', '__untagged__', or one of the 9 provinces / 'Honiara City'
  ...
  // Set of scholar names whose card is currently visible under the provinceGroup/
  ...
  function computeScholarFilterNames() {
    ...
    if (state.provinces && state.provinces.features) {
      state.provinces.features.forEach(f => provConf.set(f.properties.name, f.properties.provinceGroup));
    }
  ...
  function renderLeaders() {
    ...
    // Province → provinceGroup map (from fiji-provinces.geojson) so we can
    // classify each scholar's paternal province into a provinceGroup.
    const provConf = new Map();
    if (state.provinces && state.provinces.features) {
      state.provinces.features.forEach(f => provConf.set(f.properties.name, f.properties.provinceGroup));
    }
    ...
    // The ProvinceGroup Total pill uses this to show "N of M" once a filter narrows
    ...
    // Recompute the provinceGroup breakdown counts for the summary bar based on
    ...
    // discipline, provinceGroup, province, country/uni of study or work)
  ...
  // ============================================================
  //  Summary bar — counts per provinceGroup on the CURRENT filter result
  // ============================================================
  // Any scholar filter active? Used to decide whether the ProvinceGroup Total
  ...
  // ============================================================
  //  Two-column combo control (ProvinceGroup/Province, Study, Work)
  ...
  // ProvinceGroup → gradient stops for the banner + initials background.
  // Matches Panel A/B/C provinceGroup colours.
  const CONF_GRADIENT = {
    Central: { from: '#FF5A6E', to: '#c93e50' },
    Choiseul: { from: '#4ECDE6', to: '#0891b2' },
    Guadalcanal: { from: '#FFD84A', to: '#f7b500' },
    Isabel: { from: '#6FCF97', to: '#2f9e5c' },
    'Makira-Ulawa': { from: '#B48CE6', to: '#7c4dbd' },
    Malaita: { from: '#F2994A', to: '#c9701f' },
  ...
  // reasonably likely to appear given the iTaukei/Solomon Islands diaspora — Pacific + common
  ...
  function renderScholarCard(r) {
    ...
    const isNeutral = !provinceGroup || !CONF_GRADIENT[provinceGroup];
    ...
    card.addEventListener('click', ev => {
      if(window.__solomonSharedScholar)return;
    ...
    // beneath the photo without ever running under the provinceGroup banner
    ...
    window.SolomonScholarPortal?.wireShare(card, r);
    return card;
  }

  function wireScholarSubmit(card,row){
    card.querySelector('[data-submit-info]')?.addEventListener('click',ev=>{
      ev.stopPropagation();window.SolomonScholarPortal.openUpdate(row,state);
    });
  }
  ...
  function itemMatches(item) {
    ...
    // ProvinceGroup/province dropdown filters on the scholar leaderboard also
  ...
  // V2 preview: the authoritative data source is the iTaukei/SolomonIslands_Master_file
  ...
  async function bootApp() {
    ...
    try {
      ...
      if(window.__solomonSharedScholar){document.body.textContent='The scholar profile could not load. Please reload.';document.documentElement.classList.remove('solomon-profile-loading');return;}
    ...
    if(window.__solomonSharedScholar){
      try{renderStats();renderWorldPanel();await window.SolomonScholarPortal.renderShared(state,renderScholarCard,renderItemCard);}
      catch(error){document.body.textContent='Unable to load this scholar profile: '+error.message;}
      document.documentElement.classList.remove('solomon-profile-loading');return;
    }
    ...
    // provinceGroup, verbatim explanatory notes, timestamp badge).
    // ================================================================
  ...
  const B2_VIEWS = ['solomon-islands-focused', 'all-locations'];
  const B2_META = {
  ...
  // 'authorship' shows the iTaukei/Solomon Islands-authorship stacked bar previously on B2.
  state.b1View = 'type';
  state.worldSelectedCountry = null;
  state.worldSelectedUniversity = null;
  state.worldSearchTerm = '';
  state.b2View = 'solomon-islands-focused';
  ...
  (function readB2FromHash() {
    ...
    // Backward compatibility: old Solomon Islands links inherited the iTaukei/Solomon Islands/Fiji
    // slug. Accept it once, then renderPanelB2 rewrites the hash correctly.
    const legacySlug = m[1] === 'fiji-focused' ? 'solomon-islands-focused' : m[1];
  ...
  function c3SolomonIslandsPeople_(it, scholars, role) {
    const linked = Array.isArray(it && it._linkedSolomonIslandsPeople) ? it._linkedSolomonIslandsPeople : [];
  ...
  function buildB2Rows_paternalGrouped(includeAllLocations) {
    ...
    state.snapshot.items.forEach(it => {
      ...
      const scholar = c3SolomonIslandsPeople_(it, scholars, 'lead')[0];
      if (!scholar) return;
      if (!includeAllLocations && !c3IsSolomonIslandsFocused_(it)) return;
  ...
  function buildB2Rows_coauthor(includeAllLocations) {
    ...
    state.snapshot.items.forEach(it => {
      ...
      const coScholars = c3SolomonIslandsPeople_(it, scholars, 'co');
  ...
  function buildB2Rows_split(includeAllLocations) {
    ...
    state.provinces.features.forEach(f => {
      rows.set(f.properties.name, {
        name: f.properties.name, conf: f.properties.provinceGroup,
        lead: 0, co: 0, total: 0,
        leadTypes: {}, coTypes: {},
        isConfirmed: true
      });
    ...
    state.snapshot.items.forEach(it => {
      ...
      if (!state.b2TypeSet.has(vt)) return;
      const role = itaukeiAuthorship(it);
      const firstScholar = role === 'lead' ? c3SolomonIslandsPeople_(it, scholars, 'lead')[0] : null;
  ...
  function buildC3DivisionRows_(includeAllLocations, authMode) {
    ...
    state.snapshot.items.forEach(it => {
      ...
      const people = c3SolomonIslandsPeople_(it, scholars, peopleRole);
  ...
  function buildB2Rows_studyProvince_allAuthors() {
    const rows = new Map();
    state.provinces.features.forEach(f => {
      rows.set(f.properties.name, { name: f.properties.name, conf: f.properties.provinceGroup, total: 0, types: {} });
    });
  ...
  function buildB2AuthorsPill(currentType) {
    ...
    sel.addEventListener('change', () => {
      state.b2View = sel.value === 'all' ? 'all-authors' : 'solomon-islands-focused';
      renderPanelB2();
    });
  ...
  function renderPanelB2() {
    ...
    const meta = B2_META[view] || B2_META['solomon-islands-focused'];
    ...
    // The pill controls scope (Solomon Islands-focused vs Solomon Islands + International);
    ...
    if (view === 'solomon-islands-focused' || view === 'all-locations') {
      if (auth === 'first') {
        let rows = b2Division
  ...
  function renderPanelB2Blurb(el, ctx) {
    ...
    const auth = ctx.auth;
    const scopeWord = ctx.includeAll ? 'across all locations' : 'in Solomon Islands-focused research';
  ...
  function renderAuthorshipInto(host, rows, opts) {
    ...
    ordered.forEach(r => {
      // Column 1 — province label + provinceGroup dot + total-only click target
      const label = document.createElement('div');
      label.className = 'db-bars__prov';
  ...
  function computeImpactData(filter) {
    const conf = {
                   Central: {fiji:0, intl:0, scholars:new Set()},
                   Choiseul: {fiji:0, intl:0, scholars:new Set()},
                   Guadalcanal: {fiji:0, intl:0, scholars:new Set()},
                   Isabel: {fiji:0, intl:0, scholars:new Set()},
                   'Makira-Ulawa': {fiji:0, intl:0, scholars:new Set()},
                   Malaita: {fiji:0, intl:0, scholars:new Set()},
    ...
    // Look up each iTaukei scholar's paternal provinceGroup from their filled
    // profile. Empty provinceGroup = we can't attribute their work to a group.
    ...
    state.snapshot.items.forEach(it => {
      ...
      // Determine the paternal provinceGroup from the lead iTaukei author. We
  ...
  function renderImpactView(filter) {
    ...
    const confs = [
      { name: 'Central', color: '#FF5A6E' },
      { name: 'Choiseul', color: '#4ECDE6' },
      { name: 'Guadalcanal', color: '#FFD84A' },
      { name: 'Isabel', color: '#6FCF97' },
      { name: 'Makira-Ulawa', color: '#B48CE6' },
      { name: 'Malaita', color: '#F2994A' },
        ...
        <div class="impact-conf" style="border-top-color:${c.color};">
          ...
          <div class="impact-conf__bars">
            <div class="impact-bar">
              ...
              <div class="impact-bar__col impact-bar__col--fiji" style="height:${Math.max(4, (d.fiji / max) * MAX_H)}px;" data-tooltip="${d.fiji} Solomon Islands-focused item${d.fiji === 1 ? '' : 's'}"></div>
              <div class="impact-bar__label">Solomon Islands-focused</div>
            </div>
            <div class="impact-bar">
  ...
  // ==========================================================================
  // PANEL B4 — Where Solomon Islands research has been undertaken
  //
  // Data model:
  ...
  const B3_COUNTRY_COORDS = {
    ...
    'Solomon Islands':          { lat: -21.1789, lng: -175.1982, region: 'Pacific' },
  ...
  // overview instead of plotting Solomon Islands at the invisible -175° copy.
  ...
  const B3_SOLOMONISLANDS_DETAIL_ZOOM = 4;

  function b3SolomonIslandsDetailRecords(countryRecord) {
    if (!countryRecord || countryRecord.country !== 'Solomon Islands') return [];
    ...
    const addItems = (items, role) => items.forEach(item => {
      ...
      // prevents a stale browser item from collapsing all evidence to Solomon Islands.
      ...
      const rawRows = ((state.master && state.master.geography) || []).filter(g =>
        ...
        String(g.Country || '').trim().toLowerCase() === 'solomon-islands' &&
      ...
      })) : (item._masterGeographyRows || []).filter(g => g.country === 'Solomon Islands');
      rows.forEach(g => {
        ...
        if (!coord && lookup.country) { coord = lookup.country.get(norm('Solomon Islands')); label = 'Solomon Islands-wide'; kind = 'country'; }
        if (!coord) return;
        const key = `${item.key}\u0001${kind}\u0001${norm(label)}`;
        if (seen.has(key)) return;
        seen.add(key);
        result.push({
          country: 'Solomon Islands', subLoc: label, lat: coord.lat, lng: coord.lng,
  ...
  function b3ExpandedSolomonIslandsRecords(records) {
    return records.flatMap(rec => {
      if (rec.country !== 'Solomon Islands') return [rec];
      const details = b3SolomonIslandsDetailRecords(rec);
      if (!details.length) return [rec];
      // At island-detail zoom retain the country-level publication marker as
      // well as every verified evidence row. Thus SOL-PUB0072 renders as five
      // circles: one deduplicated publication marker + four geography rows.
      return [Object.assign({}, rec, {
        subLoc: `Solomon Islands — ${rec.total} publication${rec.total === 1 ? '' : 's'}`,
        _b3Detail: true,
  ...
  function b3DetailedSolomonIslandsBounds(countryRecord) {
    const details = b3SolomonIslandsDetailRecords(countryRecord);
  ...
  function initB3Map() {
    ...
    const whereRoot = byKey.get('V3HLPDPL')
                   || cols.find(c => c.name === 'B3-Where study was done (with Solomon Islands lead & co-author)' && !c.parent)
    ...
    // Swap between the deduplicated country bubble and individual Solomon Islands
  ...
  // ProvinceGroup › Province + Authorship). Each pick sets state.b3Filter,
  ...
  function initB3ToolbarDropdowns() {
    ...
    function paintConfProv() {
      const isSolomonIslands = state.b3Filter.country === 'Solomon Islands';
      confCol.hidden = !isSolomonIslands;
      provCol.hidden = !isSolomonIslands;
      if (!isSolomonIslands) { confList.innerHTML = ''; provList.innerHTML = ''; return; }

      // Compute per-Island-Division and per-District counts against Solomon Islands.
      const solomonIslandsRec = recs.find(r => r.country === 'Solomon Islands');
      if (!solomonIslandsRec) return;
      const provOfItem = state.provincesByItem || new Map();
      const divisionOfItem = state.islandDivisionsByItem || new Map();
      const auth = state.b3Filter.authorship;
      const items = (auth === 'led') ? solomonIslandsRec.led
                 : (auth === 'others') ? solomonIslandsRec.others
                 : solomonIslandsRec.led.concat(solomonIslandsRec.others);
    ...
    function repaintAllDropdowns() {
      ...
      if (state.b3Filter.provinceGroup) parts.push(state.b3Filter.provinceGroup);
      ...
      if (clearBtn) {
        const hasFilter = !!(state.b3Filter.region || state.b3Filter.country ||
                             state.b3Filter.provinceGroup || state.b3Filter.province ||
    ...
    if (clearBtn) {
      clearBtn.addEventListener('click', (e) => {
        ...
        state.b3Filter.provinceGroup = null;
    ...
    regionPanel.addEventListener('click', (e) => {
      ...
      if (t.hasAttribute('data-region-pick')) {
        ...
        // Changing country invalidates Solomon Islands-only sub-selections.
        state.b3Filter.provinceGroup = null;
        state.b3Filter.province = null;
        ...
        state.b3Filter.provinceGroup = t.getAttribute('data-conf-pick') || null;
        state.b3Filter.province = null;
  ...
  function b3FilteredRecords() {
    ...
    return recs.map(rec => {
      ...
      // Solomon Islands-only: Island-Division/District narrowing.
      const isSolomonIslands = rec.country === 'Solomon Islands';
      if (isSolomonIslands && (filter.provinceGroup || filter.province)) {
        const matchItem = (it) => {
          const set = provOfItem.get(it.key);
          if (filter.province) return !!(set && set.has(filter.province));
          if (filter.provinceGroup) {
            const divisions = divisionOfItem.get(it.key);
            return !!(divisions && divisions.has(filter.provinceGroup));
          }
          return true;
        };
  ...
  function renderB3Layer() {
    ...
    if (bmap.getZoom() >= B3_SOLOMONISLANDS_DETAIL_ZOOM) records = b3ExpandedSolomonIslandsRecords(records);
    ...
    const markers = [];
    // The country summary and nationwide evidence share Solomon Islands' canonical
  ...
  function b3InTextCitation(item) {
    ...
    // works only counts the iTaukei/Solomon Islands scholars linked in the Authorship
  ...
  // name + venue + title, plus a village/(province) – ProvinceGroup chip when
  ...
  function b3WirePopupHovers(popupEl, rec) {
    ...
    chips.forEach(chip => {
      chip.addEventListener('mouseenter', () => {
        ...
        // Solomon Islands. Indigenous people featured beside the lead come from the
        ...
        const linkedPayload = Array.isArray(it._linkedSolomonIslandsPeople)
          ? it._linkedSolomonIslandsPeople
          : (Array.isArray(it._linkedSolomonIslandsScholarNames)
              ? it._linkedSolomonIslandsScholarNames.map(name => ({ name, kind: 'scholar' }))
              : []);
        const linkedSolomonIslandsPeople = [];
        const seenSolomonIslandsPeople = new Set();
        linkedPayload.forEach(person => {
          ...
          // from dropping the Solomon Islander scholar's canonical name and portrait.
          ...
          if (seenSolomonIslandsPeople.has(keyName)) return;
          seenSolomonIslandsPeople.add(keyName);
          linkedSolomonIslandsPeople.push({
            name: rawName,
            profile,
        ...
        const featuredSolomonIslandsPeople = rec.isLed
          ? (linkedSolomonIslandsPeople.filter(person => person.isFirstAuthor).length
              ...
              : linkedSolomonIslandsPeople.slice(0, 1))
          : linkedSolomonIslandsPeople;
        const resolvedLeadProfile = leadProfile ||
          (featuredSolomonIslandsPeople[0] && featuredSolomonIslandsPeople[0].profile) || null;
        ...
        if (rec.isLed) {
          photoUrl = (resolvedLeadProfile && resolvedLeadProfile.photo) ||
            (featuredSolomonIslandsPeople[0] && featuredSolomonIslandsPeople[0].profile && featuredSolomonIslandsPeople[0].profile.photo) || '';
        } else if (featuredSolomonIslandsPeople.length) {
          rotationUrls = featuredSolomonIslandsPeople.map(c => (c.profile && c.profile.photo) || '').filter(Boolean);
          photoUrl = rotationUrls[0] || '';
        }

        // ---- Header line: lead name + year + (for Others rows) inline Solomon Islands
        ...
        const displayLead = rec.isLed && resolvedLeadProfile
          ...
          : firstAuthor;
        let coauthInline = '';
        if (!rec.isLed && featuredSolomonIslandsPeople.length) {
          const parts = featuredSolomonIslandsPeople.map((c, i) => {
            const cls = (i % 2 === 0) ? 'is-alt-a' : 'is-alt-b';
    ...
    host.querySelectorAll('[data-b3-country]').forEach(row => {
      row.addEventListener('click', () => {
        ...
        if (name === 'Solomon Islands') {
          const detailBounds = b3DetailedSolomonIslandsBounds(summary);
        ...
        if (m && name !== 'Solomon Islands') setTimeout(() => m.openPopup(), 350);
      });
    });
  }
})();
