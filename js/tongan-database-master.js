...
(function () {
  ...
  function zoomToWorldUniversity(uniName) {
    ...
    if (rendered.length) {
      ...
      state.worldMap.setView(rendered[0], p ? (8) : 8, { animate: true });
      return;
    }
    const grad = state.graduateStudies || { worldPoints: [] };
    const p = (grad.worldPoints || []).find(x => x.university === uniName);
  ...
  function renderImpactView(filter) {
    ...
    let note = `Showing ${filterLabel}. “Tonga-focused” means the item is tagged to at least one Tonga district in the Master file; “International” means it isn’t. Island Division is attributed via the lead Tongan author’s paternal district.`;
