/**
 * Demo-mode gate for the Solomon Islands Scholar Database (dashboard).
 *
 * Sister clone of js/demo-gate.js (iTaukei). Uses its own baked passcode
 * and its own HMAC demo-token signing key, both distinct from the iTaukei/Solomon Islands
 * originals — see SOLOMON-DASHBOARD-BUILD-NOTES.md at the repo root for the
 ...
 * The .enc data files stay encrypted with their own Solomon Islands passcode-derived
 * key (distinct from the iTaukei/Solomon Islands/Solomon Islands sister system's), but that passcode is now baked into this file
...
(function () {
  ...
  var REVOKE_PATH = 'data/solomon-revoked-demos.json';
  ...
  var ENC_FILES = {
    ...
    // Master-file dashboard code path (js/solomon-database-master.js never
    // calls fetchJson('data/world-universities.json') — same as the
    // upstream iTaukei file). Mapped here to a Solomon Islands-prefixed target
    ...
    'data/world-universities.json':      'data/solomon-world-universities.json.enc',
    'data/solomon-islands-provinces.json':         'data/solomon-islands-provinces.json.enc',
    'data/solomon-scholar-profiles.json': 'data/solomon-scholar-profiles.json.enc',
    'data/solomon-last-sync.json':        'data/solomon-last-sync.json.enc',
    'data/solomon-graduate-studies.json': 'data/solomon-graduate-studies.json.enc',
    'data/solomon-scholar-insights.json': 'data/solomon-scholar-insights.json.enc',
    // Master-file V2 preview snapshots (refreshed every 2h by
    // .github/workflows/refresh-solomon-master-file.yml, once created).
    // Only the V2 preview page fetches these; the production dashboard
    // does not.
    'data/solomon-master-scholars.json':     'data/solomon-master-scholars.json.enc',
    'data/solomon-master-publications.json': 'data/solomon-master-publications.json.enc',
    'data/solomon-master-authorship.json':   'data/solomon-master-authorship.json.enc',
    'data/solomon-master-researcher-authorship.json':
      'data/solomon-master-researcher-authorship.json.enc',
    'data/solomon-master-grad-degrees.json': 'data/solomon-master-grad-degrees.json.enc',
    'data/solomon-master-mobility.json':     'data/solomon-master-mobility.json.enc',
    'data/solomon-master-geography.json':    'data/solomon-master-geography.json.enc',
    'data/solomon-master-aggregates.json':   'data/solomon-master-aggregates.json.enc',
    ...
    'data/solomon-master-worldpoints.json':  'data/solomon-master-worldpoints.json.enc',
    'data/solomon-last-master-sync.json':     'data/solomon-last-master-sync.json.enc',
    ...
    // ?src=master.
    'data/solomon-body-composition-master.json': 'data/solomon-body-composition-master.json.enc',
    'data/solomon-body-composition.json':        'data/solomon-body-composition.json.enc',
    ...
    'data/solomon-scholar-enrichment.json':   'data/solomon-scholar-enrichment.json.enc',
  ...
  async function boot(onReady) {
    const profileToken = new URLSearchParams(location.search).get('solomonProfile');
    if (profileToken !== null) {
      try {
        if (!/^[0-9a-f]{40}$/.test(profileToken)) throw new Error('Invalid scholar link.');
        const response = await fetch('data/solomon-share-tokens.json', {cache:'no-cache'});
        if (!response.ok) throw new Error('Scholar links are not yet available.');
        const doc = await response.json();
        if(doc.country!=='Solomon Islands')throw new Error('Wrong country scholar links.');
        const sid = Object.keys(doc.m || {}).find(id => doc.m[id] === profileToken && /^SOL-S\d{4}$/.test(id));
        if (!sid) throw new Error('This scholar link was not found.');
        window.__solomonSharedScholar = {scholarId:sid, token:profileToken};
        cachedPasscode = BAKED_PASSCODE;
        onReady();
      } catch (error) {
        document.body.replaceChildren();
        const message = document.createElement('p'); message.textContent = error.message;
        document.body.appendChild(message); document.documentElement.classList.remove('solomon-profile-loading');
      }
      return;
    }
