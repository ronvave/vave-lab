/*
 * Tongan Master-file → Zotero-shape adapter
 * ==========================================
 *
 * Sister clone of js/master-file-adapter.js (iTaukei), pointed at the
 * Tongan Master-file JSON snapshots (produced by a Tongan-specific
 * transformer script pointed at spreadsheet ID
 * 1lh6wOFcg2GiFe2YylgxM5cvLOdumdbCrHDLQk87rjRI). Fully separate from the
 * iTaukei system — never reads iTaukei-prefixed data files.
 *
 ...
 * the production `js/tongan-database-master.js` expects (itself a clone of
 * `js/itaukei-database-master.js`), but reading its ground truth from the
 * Tongan Master-file JSON snapshots.
 *
 ...
 *
 * Key semantic rules applied here (per Master-file spec, Tonga crosswalk):
 *   - Scholar identity → Scholar ID (TON-S####).
 *   - Publication identity → Publication ID / BibTeX Key.
 *   - Tongan-associated status → Authorship bridge link to a Scholar ID.
 *   - Lead author → Author Position = 1 OR Is First Author? = Yes.
 *   - Institution counts → C_Uni name only (O_Uni is historical metadata).
 *   - Tonga publication geography → Master-file 23 district one-hots +
 *     "Tonga - no district specified" + "Unsure".
 *   - Island Divisions: Tongatapu / Vava'u / Ha'apai / 'Eua / Ongo Niua
 *     (see DISTRICT_TO_DIVISION). "Confederacy" is a Fijian-specific
 *     concept and is never used for Tonga — this file exposes the exact
 *     same internal property names (PROVINCE_TO_CONFED, CONFEDERACIES,
 *     .confederacy, .paternalProvince, etc.) that the cloned dashboard
 *     JS (js/tongan-database-master.js) already expects, so the
 *     downstream chart/filter/tooltip logic runs unmodified — but every
 *     *value* held in those properties is Tonga's own District / Island
 *     Division data, and every *display string* the dashboard shows the
 *     user is relabeled "District" / "Island Division" (see
 *     js/tongan-database-master.js and js/tongan-panel-overrides.js).
 *
 * The Master-file JSON files loaded here all pass through the encrypted
 * gate (js/tongan-demo-gate.js). Absence of the .enc files → the caller
 * sees a graceful "data not yet available" state.
 */
...
(function () {
  ...
  // -------------------------------------------------------------------
  // Constants — Tonga's 5 Island Divisions → 23 Districts (2021 Census,
  // Tonga Statistics Department). Mirrors scripts/tongan_master_file_config.py.
  // Internal variable names (CONFEDERACIES, PROVINCE_TO_CONFED, PROVINCES)
  // are kept identical to the iTaukei adapter on purpose — the cloned
  // dashboard JS reads these exact names via window.MasterFileAdapter /
  // window.TonganMasterFileAdapter.constants. Only the *contents* change.
  // "Confederacy" concept → Tonga's "Island Division"; "Province" → "District".
  // -------------------------------------------------------------------
  ...
  var PROVINCES = Object.keys(PROVINCE_TO_CONFED);           // 23 districts
  var PROVINCE_UNSPEC = 'Tonga - no district specified';
  var PROVINCE_UNSURE = 'Unsure';

  // Human-readable aliases exposed alongside the legacy names above so any
  // Tonga-aware code (tongan-database-master.js / tongan-panel-overrides.js)
  // can read self-documenting names instead of the Fiji-shaped originals.
  var ISLAND_DIVISIONS = CONFEDERACIES;
  var DISTRICT_TO_DIVISION = PROVINCE_TO_CONFED;
  ...
  function loadRawMaster() {
    ...
    return Promise.all([
      fetchJson('data/tongan-master-scholars.json'),
      fetchJson('data/tongan-master-publications.json'),
      fetchJson('data/tongan-master-authorship.json'),
      ...
      fetchJson('data/tongan-master-researcher-authorship.json')
  ...
  function buildZoteroSnapshot(master) {
    ...
    var COL_ROOT_ITAUKEI     = 'TONAROOT';    // "Tongan authors (>N papers)"
    var COL_BY_WITH          = 'TONABYWI';    // "By or with Tongan authors"
    var COL_ROOT_C1_PROV     = 'C1ROOTKY';    // publication-location root
    var COL_NONPROV_FIJI     = 'AREH32KK';    // "_Non-District/Tonga"
    ...
    // Every scholar becomes a fake Tongan author sub-collection so the
    // existing "Tongan author" walkers pick them up.
    var scholarKeyById = {};
    var scholarNameById = {};
    ...
    var rootCollections = [
      ...
      { key: COL_NONPROV_FIJI, name: '_Non-District/Tonga',       parent: COL_ROOT_C1_PROV },
      { key: COL_ROOT_PATERNAL,name: 'Paternal District',           parent: null },
      { key: COL_ROOT_DISCIPLINE, name: 'Discipline',               parent: null },
      { key: COL_ROOT_THESIS_UNI, name: 'B2-Tongan Thesis by Country/Universities', parent: null },
      { key: COL_ROOT_B3_WHERE, name: 'B3-Where study was done (with Tongan lead & co-author)', parent: null }
    ...
    return {
      ...
      _rootKeys: {
        tonganAuthors: COL_ROOT_ITAUKEI,
  ...
  // -------------------------------------------------------------------
  // Build a tonga-districts.json flat descriptor from the district → island-division map.
  // -------------------------------------------------------------------
  ...
  // -------------------------------------------------------------------
  // Build a tonga-districts.geojson stub — production code uses it for the
  ...
  function buildGeoJson(snap) {
    return fetchJson('data/tonga-districts.geojson').then(function (geo) {
  ...
  function buildProfiles(master, snap) {
    var scholars = master.scholars.map(function (s) {
      ...
      // Tonga schema field names differ from the iTaukei sheet: District
      // Paternal/Maternal (was Province Paternal/Maternal), Specific Island
      // Paternal/Maternal (was Island Paternal/Maternal, now independently
      // curated since Tonga districts like Lulunga span multiple physical
      // islands), Village/Town Paternal/Maternal (Kolo) (was Village
      // Paternal/Maternal). Tonga's official hierarchy has no layer between
      ...
      return {
        ...
        // the iTaukei adapter so the cloned dashboard JS's render/filter
        // logic runs unmodified. For Tonga: paternalProvince/maternalProvince
        // hold the DISTRICT value (displayed as "District" in the UI, never
        // "Province"); paternalDistrict/maternalDistrict hold the dropped
        // tikina-level field and are always '' for Tonga (no Tonga
        // equivalent — see crosswalk §4). paternalIsland/maternalIsland hold
        // Tonga's "Specific Island" (independently curated, since districts
        // like Lulunga span multiple islands). paternalVillage/maternalVillage
        // hold Tonga's "Village/Town (Kolo)".
        ...
        // Ambiguity A — resolved by user decision 2026-08-28 to split
        // Paternal/Maternal, mirroring the geography columns). Never
        // inferred from geography, surname, or title.
        estateAffiliationPaternal: cleanSentinel_(s["Estate / Chiefly Affiliation Paternal (Tofi'a)"]),
        estateAffiliationMaternal: cleanSentinel_(s["Estate / Chiefly Affiliation Maternal (Tofi'a)"]),
        haaLineagePaternal:        cleanSentinel_(s["Ha'a / Lineage Paternal"]),
        haaLineageMaternal:        cleanSentinel_(s["Ha'a / Lineage Maternal"]),
        kaingaPaternal:            cleanSentinel_(s['K\u0101inga Paternal']),
        kaingaMaternal:            cleanSentinel_(s['K\u0101inga Maternal']),
        selfIdentifiedHomePaternal: cleanSentinel_(s['Self-identified Home / Community Affiliation Paternal']),
        selfIdentifiedHomeMaternal: cleanSentinel_(s['Self-identified Home / Community Affiliation Maternal']),
    ...
    return {
      ...
      hiddenScholars: [],
      notItaukeiAuthors: []  // legacy key name kept for downstream compatibility; holds not-Tongan exclusions
    };
  }
  ...
  // iTaukei adapter — because js/tongan-database-master.js (a byte-level
  ...
  // separate HTML documents. A `window.TonganMasterFileAdapter` alias is
  // also exposed for any Tonga-aware code that wants an unambiguous name.
  var TONGAN_ADAPTER_API = {
    load: loadFromMaster,
    constants: {
      ...
      TYPE_MAP: TYPE_MAP,
      // Tonga-named aliases of the same constants (self-documenting).
  ...
  window.MasterFileAdapter = TONGAN_ADAPTER_API;
  window.TonganMasterFileAdapter = TONGAN_ADAPTER_API;
})();
