...
(function () {
  ...
  var SESSION_KEY   = 'solomonlab_admin_master_session';
  var GH_TOKEN_KEY  = 'solomonlab_gh_token';
  ...
  // `refresh-solomon-master-file.yml` GitHub Action; without a dispatch, the public
  ...
  var ENRICHMENT_ENC = 'data/solomon-scholar-enrichment.json.enc';
  var INSIGHTS_ENC   = 'data/solomon-scholar-insights-master.json.enc';
  ...
  async function loadOldZoteroCountsBySid () {
    ...
    try {
      scholarProfiles = await window.dbGate.fetchJson('data/solomon-scholar-profiles.json');
    } catch (e) {
    ...
    // solomon-scholar-profiles.json is name-keyed (V1 shape). Match to Master rows by
  ...
  function renderScholars () {
    ...
    if (state.filterIslandDivision) {
      rows = rows.filter(function (s) {
        return String(s['Province/City Area'] || '').trim() === state.filterIslandDivision;
      });
    }
  ...
  function scholarRowHtml (s) {
    ...
    var conf = (s['Province/City Area'] || '').trim();
    ...
    try {
      ...
      results.forEach(function (r, j) {
        ...
        if (r.status === 'ok') {
          ...
          // 2-hour cron. The `refresh-solomon-master-file.yml` dispatch below
  ...
  async function performNonMasterPush (sid, opts) {
    ...
    try {
      ...
      // present in solomon-scholar-enrichment.json.enc are preserved verbatim so a
      ...
      //    sha race, so concurrent refresh-solomon-master-file runs (salt-only
      ...
      if (needsPublicRefresh) {
        log('Master-backed public field changed — dispatching refresh-solomon-master-file.yml…', 'warn');
  ...
  //           handles the salt-only rewrite that refresh-solomon-master-file.yml
  ...
  //       the refresh-solomon-master-file workflow ran twice (12:55 and 13:00) and
  //       rewrote solomon-scholar-enrichment.json.enc with fresh salts. The old
  ...
  var _dbSessionKey = 'solomonlab.db.session.v1';
  ...
  async function pushEncryptedJsonMerged (path, plaintextUrl, applyEdit, commitMsg) {
    ...
    // than blindly re-PUTting stale bytes. With refresh-solomon-master-file.yml
  ...
  //
  // Then step 2: dispatch refresh-solomon-master-file.yml (the Solomon Islands
  ...
  //
  // Files scanned:
  //   - solomon-islands-research-database-master.html  (public V2 dashboard, once built)
  //   - admin-solomon-islands-master.html               (this admin, so admin
  ...
  var FORCE_CACHE_BUST_FILES = [
    'solomon-islands-research-database-master.html',
    'admin-solomon-islands-master.html'
  ];
  ...
  // file that contains Solomon Islands diacritics such as the 'okina ('\u02bb) and
  ...
  async function dispatchRefresh (opts) {
    ...
    try {
      // refresh-solomon-master-file.yml (deployed alongside iTaukei's
      // refresh-master-file.yml) runs scripts/solomon_master_file_transformer.py
      // against the Solomon Islands Master Sheet and re-encrypts data/solomon-master-*.json.
      ...
      // on the Solomon Islands Sheet, or that run will fail even though this
      ...
      var url = 'https://api.github.com/repos/' + GH_OWNER + '/' + GH_REPO + '/actions/workflows/refresh-solomon-master-file.yml/dispatches';
      ...
      if (res.status === 204) {
        ...
        log('Dispatched refresh-solomon-master-file.yml', 'ok');
  ...
  function computeRepairPriority (canonicalTotal, oldTotal, reason) {
    ...
    // repair value: e.g. a scholar SOL-S0379 with old 68 / current 0.
  ...
  function exportGapsCsv () {
    ...
    var header = [
      ...
      'Alive/Deceased', 'Discipline', 'Paternal Province/City Area', 'Roster Tier'
    ];
    ...
    a.href = url;
    a.download = 'solomon-master-authorship-linkage-gaps-' + new Date().toISOString().slice(0, 10) + '.csv';
  ...
  function wireControls () {
    ...
    if (migWriteBtn) migWriteBtn.addEventListener('click', async function () {
      ...
      if (!confirm('Publish ' + migState.lastResult.counts.v2TotalAfter + ' Scholar-ID-keyed insights to data/solomon-scholar-insights-master.json.enc?')) return;
      migWriteBtn.disabled = true;
      try {
        await window.adminInsightsMigration.publishV2(migState.lastResult.v2Doc);
        toast('Published data/solomon-scholar-insights-master.json.enc.', 'ok');
      } catch (e) {
    ...
    function refreshWritebackStatus(pingResult) {
      ...
      if (!configured) {
        wbStatus.textContent = 'not yet configured';
        wbStatus.style.background = 'var(--cream)'; wbStatus.style.color = 'var(--muted)';
        wbDetail.textContent = 'Writeback endpoint not yet configured. The Solomon Islands Apps Script backend (apps-script/solomon-master-writeback.gs) has not been deployed yet -- see docs/SOLOMON-ISLANDS-APPS-SCRIPT-DEPLOY.md. Paste the Web app URL + shared secret above once deployed, then click Test connection.';
        return;
      }
    ...
    // Refresh from Sheet (top bar). Dispatches refresh-solomon-master-file.yml only
