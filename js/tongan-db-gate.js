/**
 * Passcode gate for the Tongan Scholar Database.
 *
 * This is a fully separate, sister clone of js/db-gate.js built for the
 * Tongan Scholar Database admin/dashboard system. It uses its own brand-new
 * passcode and its own VERIFIER_HASH_HEX \u2014 it never reuses the iTaukei
 * passcode, hash, or ENC_FILES map. It is not read by, and does not read,
 * the iTaukei localStorage session key.
 *
...
(function () {
  ...
  var STORAGE_KEY = 'tonganlab.db.session.v1';
  ...
  // the on-disk .enc URL. All Tongan-prefixed \u2014 never overlaps with the
  ...
  var ENC_FILES = {
    ...
    // These are consumed by tongan-research-database-master.html /
    // js/tongan-database-master.js and refreshed by the Tongan variant of
  ...
  function buildLockScreenHtml() {
    return ''
      ...
      +     '<p class="db-gate__lede">This Tongan Scholar Database is being previewed with a small group of colleagues while feedback is gathered. If Prof. Ron Vave has shared a passcode with you, enter it below.</p>'
