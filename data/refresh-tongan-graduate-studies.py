#!/usr/bin/env python3
"""Sister clone of data/refresh-graduate-studies.py for the Tongan Scholar
Database. Extract Tongan graduate-studies info from the Zotero snapshot.

Uses the same underlying Zotero group as the iTaukei system (Ron's single
research library covers both projects) but reads/writes Tongan-prefixed
files only — never touches data/itaukei-*.

Panel A ("World-Tongan graduate studies") is powered by this file's output,
data/tongan-graduate-studies.json.

Source of truth
---------------
The Zotero collection tree
    Tongan Thesis by Country/Universities (key: 9XHGQJE6)
       ├── Australia
...
Tongan-graduate work. The scholar name comes from the item's first
...
Previously the iTaukei version of this script only counted theses whose
author had a personal sub-collection under "Tongan authors (>2 papers)" —
this missed every one-degree scholar and dropped a meaningful share of
theses. It also relied on a hardcoded university → country dict; the tree
gives us both directly.

Output: data/tongan-graduate-studies.json  (public dashboard reads this).
"""
...
SNAP = REPO / "data" / "tongan-zotero-snapshot.json"
OUT = REPO / "data" / "tongan-graduate-studies.json"
...
# source of truth for which theses are Tongan-graduate work.
# Match by stable Zotero key first (survives renames like the B2- prefix
# convention), then fall back to name matching for legacy snapshots. Names
# match the Tongan adapter's collection-name renaming (js/tongan-database-
# adapter.js), which emits "B2-Tongan Thesis by Country/Universities".
THESIS_ROOT_KEY = "9XHGQJE6"
THESIS_ROOT_NAME_CANDIDATES = (
    "B2-Tongan Thesis by Country/Universities",
    "Tongan Thesis by Country/Universities",
)
...
# on every worldPoints entry so js/tongan-database-master.js can build the
...
def load_world_universities():
    ...
    under "Tongan Thesis by Country/Universities" whose only country signal
