...
const props=new Map([['SOLOMON_GOOGLE_CLIENT_ID','fixture-client'],['SOLOMON_OWNER_EMAIL','owner@example.edu'],['SOLOMON_REVIEWER_EMAILS','reviewer@gmail.com'],['SHARED_SECRET','fixture-secret'],['WRITE_ENABLED','true']]);
...
vm.createContext(ctx);vm.runInContext(fs.readFileSync('apps-script/vendor/solomon-jwt.gs','utf8'),ctx);vm.runInContext(fs.readFileSync('apps-script/solomon-master-writeback.gs','utf8'),ctx);
ctx.solomonGoogleKeys_=()=>[jwk];ctx.jsonOut_=x=>x;
...
ctx.solomonApproveScholar_=()=>{writes++;return{status:'ok',actor:ctx.ACTOR_LABEL}};ctx.handleResolvePublicationGeographySubmission_=ctx.solomonApproveScholar_;
...
props.set('SOLOMON_REVIEWER_EMAILS','');assert.equal(post({action:'reviewCapabilities',idToken:token}).status,'unauthorized','revocation applies to current token');
props.set('SOLOMON_REVIEWER_EMAILS','reviewer@gmail.com');assert.equal(post({action:'reviewCapabilities',idToken:token}).role,'admin');
...
props.set('SOLOMON_OWNER_EMAIL','');assert.equal(post({action:'reviewCapabilities',idToken:token}).status,'unauthorized','owner setting required');
// A reviewer cannot claim an uploaded headshot was published.
ctx.SOLOMON_REQUEST_ROLE='admin';ctx.solomonWithSubmission_=(b,fn)=>fn({}, {'Scholar ID':'SOL-S0001'});ctx.solomonReviewPlan_=()=>({items:[{kind:'file',key:'photo',selected:true,state:'pending',field:'headshot'}]});
assert.throws(()=>ctx.solomonRecordAttachment_({fileId:'photo',disposition:'published',evidence:'img/scholars/SOL-S0001.jpg'}),/Owner/);
