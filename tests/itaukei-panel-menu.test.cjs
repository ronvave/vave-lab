...
(async()=>{
 ...
 const pills=nav.querySelectorAll('.db-panel-menu__pill');
 nav.getBoundingClientRect=()=>({top:76,left:0,bottom:138,height:62});
 pills[1].getBoundingClientRect=()=>({top:87,left:331,bottom:126,width:100,height:39});
 pills[1].click();
 assert.equal(nav.querySelector('.db-panel-menu__dropdown').style.top,'54px');
 assert.equal(nav.querySelector('.db-panel-menu__dropdown').style.left,'331px');
 assert.equal(76+parseFloat(nav.querySelector('.db-panel-menu__dropdown').style.top)-126,4);
 assert.match(fs.readFileSync('css/itaukei-panel-menu.css','utf8'),/menu__dropdown\{position:absolute/);
assert.equal(nav.querySelectorAll('.db-panel-menu__dropdown a').length,5);assert.equal(pills[1].getAttribute('aria-expanded'),'true');
